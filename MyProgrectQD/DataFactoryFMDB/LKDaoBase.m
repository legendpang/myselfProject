...
-(NSArray*)syncSearchAll {
    return [self syncSearchWhere:nil orderBy:nil offset:0 count:15];
}
...
-(NSArray *)syncSearchWhere:(NSString*)where
{
    return [self syncSearchWhere:where orderBy:nil offset:0 count:15];
}
...
-(NSArray *)syncSearchWhere:(NSString*)where orderBy:(NSString*)orderBy offset:(int)offset count:(int)count
{
    NSMutableString* query = [NSMutableString stringWithFormat:@"select rowid,* from %@ ",[self.class getTableName]];
    if(where != nil && ![where isEmptyWithTrim])
    {
    ...
    [self sqlString:query AddOder:orderBy offset:offset count:count];
    return [self syncRawSql:query withArgumentsInArray:nil];
}
...
-(NSArray *)syncSearchWhereDic:(NSDictionary*)where orderBy:(NSString *)orderby offset:(int)offset count:(int)count
{
    NSMutableString* query = [NSMutableString stringWithFormat:@"select rowid,* from %@ ",[self.class getTableName]];
    NSMutableArray* values = [NSMutableArray arrayWithCapacity:0];
    if(where !=nil&& where.count>0)
    ...
    [self sqlString:query AddOder:orderby offset:offset count:count];
    return [self syncRawSql:query withArgumentsInArray:values];
}

-(void) rawSql:(NSString*) sql callback:(void(^)(NSArray *))block
{
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         NSString* query = [sql stringByReplacingOccurrencesOfString:@"#TABLE#"
                              withString:(NSString*)[self.class getTableName]];
         FMResultSet* set =[db executeQuery:query withArgumentsInArray:nil];
         [self executeResult:set block:block];
     }];
}

-(NSArray*) syncRawSql:(NSString*) sql withArgumentsInArray:(NSArray*)values
{
    FMDatabase* db = [bindingQueue database];
    NSString* query = [sql stringByReplacingOccurrencesOfString:@"#TABLE#"
                                                     withString:(NSString*)[self.class getTableName]];
    FMResultSet* set =[db executeQuery:query withArgumentsInArray:values];
    return [self executeResult:set];
}
    ...
    if (set) {
        while ([set next]) {
            ...
            for (int i=0; i<self.columeNames.count; i++) {
                ...
                }else if([columeType isEqualToString:@"NSMutableArray"]||[columeType isEqualToString:@"NSArray"])
                {
                    NSArray *temp = nil;
                    NSString *cName = [set stringForColumn:columeName];
                    if ([cName rangeOfString:@"\n"].location != NSNotFound) {
                        //W("attention!!");
                    }
                    cName = [cName stringByReplacingOccurrencesOfString:@" " withString:@""];
                    cName = [cName stringByReplacingOccurrencesOfString:@"\n" withString:@""];
                    cName = [cName stringByReplacingOccurrencesOfString:@"\"" withString:@""];
                    cName = [cName stringByReplacingOccurrencesOfString:@"(" withString:@""];
                    cName = [cName stringByReplacingOccurrencesOfString:@")" withString:@""];
                    cName = [cName stringByReplacingOccurrencesOfString:@"," withString:@":"];
                    temp  = [cName componentsSeparatedByString:@":"];
                    [bindingModel setValue:temp forKey:columeName];
                }
                else if([columeType isEqualToString:@"UIImage"])
                {
                    ...
                    {
                        UIImage* image = [UIImage imageWithContentsOfFile:[SandboxFile GetPathForDocuments:filename inDir:@"dbImages"]];
                        [bindingModel setValue:image forKey:columeName];
                    }
                }
    ...
    [bindingQueue inDatabase:^(FMDatabase* db)
     {
         ...
         for (int i=0; i<self.columeNames.count; i++) {
             ...
             else if([value isKindOfClass:[NSDate class]])
             {
                 ...
                 NSMutableString* extrastr = [NSMutableString stringWithFormat:@""];
                 NSMutableArray *temp = (NSMutableArray *)value;
                 for (int i=0; i<temp.count; i++) {
                     if (i==0) {
                         [extrastr appendString:[temp objectAtIndex:i]];
                     } else {
                         [extrastr appendFormat:@":%@",[temp objectAtIndex:i]];
                     }
                 }
                 value = extrastr;
             }
         ...
         if(execute == NO)
         {
             //D("database insert fail %@",NSStringFromClass(model.class));
         }
     }];
}
-(BOOL)syncInsertToDB:(NSObject<LKModelBaseInteface>*)model
{
    FMDatabase* db = [bindingQueue database];
    NSDate* date = [NSDate date];
    NSMutableString* insertKey = [NSMutableString stringWithCapacity:0];
    ...
    for (int i=0; i<self.columeNames.count; i++) {
        ...
        else if([value isKindOfClass:[NSDate class]])
        {
            ...
            for (int a=0; a<temp.count; a++) {
                if (a==0) {
                    [extrastr appendString:[temp objectAtIndex:a]];
                } else {
                    [extrastr appendFormat:@":%@",[temp objectAtIndex:a]];
                }
            }
            value = extrastr;
        }
...
//
         ...
         if(execute == NO)
         {
             //D("database update fail %@   ----->rowid: %lld",NSStringFromClass(model.class),model.rowid);
         }
     }];
...
-(BOOL)syncUpdateToDB:(NSObject<LKModelBaseInteface>*)model
{
    FMDatabase* db = [bindingQueue database];
    NSDate* date = [NSDate date];
    NSMutableString* updateKey = [NSMutableString stringWithCapacity:0];
    ...
    if(execute == NO)
    {
        //W("database update fail %@   ----->rowid: %d",NSStringFromClass(model.class),model.rowid);
    }
    return execute;
}
...
//批量修改（数据库中无数据则进行插入）
-(void)updateBatchDataToDB:(NSArray *)dataArray
{
    FMDatabase* db = [bindingQueue database];
    [db beginTransaction];
    BOOL isRollBack = NO;
    @try{
        for (id <LKModelBaseInteface> model in dataArray) {
            ...
            //查询该条数据是否存在
            NSString* rowCountSql = [NSString stringWithFormat:@"select count(rowid) from %@ where %@",[self.class getTableName],[NSString stringWithFormat:@"%@ = '%@'",model.primaryKey,[self safetyGetModel:model valueKey:model.primaryKey]]];
            ...
            if(exists){//存在，则更新
                ...
                NSString* insertSQL = [NSString stringWithFormat:@"insert into %@(%@) values(%@)",[self.class getTableName],insertKey,insertValuesString];
                BOOL execute = [db executeUpdate:insertSQL withArgumentsInArray:updateValues];
                model.rowid = db.lastInsertRowId;
                if(execute == NO)
                {
                    //D("database insert fail %@",NSStringFromClass(model.class));
    ...
    @catch (NSException *exception) {
        //D("%@批量插入rollback", [self.class getTableName]);
        isRollBack = YES;
        [db rollback];
    }
    @finally {
        if (!isRollBack) {
            //D("%@批量插入commit", [self.class getTableName]);
            [db commit];
...
-(BOOL)syncDeleteToDB:(NSObject<LKModelBaseInteface>*)model
{
    FMDatabase* db = [bindingQueue database];
    NSString* delete;
    BOOL result;
...
{
    if (valueKey == nil) {
        //W("DataFactory safetyGetModel error. valueKey = nil ");
        return @"";
    }
...
+ (void)getSelfPropertys:(NSMutableArray *)pronames protypes:(NSMutableArray *)protypes isGetSuper:(BOOL)isGetSuper
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        }else if([propertyType hasPrefix:@"Ta"])
        {
            [protypes addObject:@"char"];
        } else {
           // W("can't process type: %@",propertyType);
            hasFindType = NO;
        }
